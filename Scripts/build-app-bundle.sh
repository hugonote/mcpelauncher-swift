...
SWIFT_PROTOBUF_LICENSE="$(find "$PACKAGE_DIR/.build/checkouts/swift-protobuf" -maxdepth 1 -name 'LICENSE*' -type f -print -quit 2>/dev/null || true)"
if [[ -n "$SWIFT_PROTOBUF_LICENSE" ]]; then
  cp "$SWIFT_PROTOBUF_LICENSE" "$APP_DIR/Contents/Resources/Licenses/SwiftProtobuf-LICENSE.txt"
fi
